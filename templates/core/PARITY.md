...
|---|---|---|
| New site › **Request** tab | `nsCreate` app.js:1120 | customer's site request is silently discarded — no `POST /site-requests` |
...
| `POST /site-requests`, `/site-requests/continue`, `/site-requests/delete` | customer requests a site; operator advances 1→2→3 and closes it out | v3's Request tab fires the mock `runJob('site-request')` (app.js:1120) and there is no operator queue at all (`grep -rn "site-requests" templates/core/` → nothing). v1: markup 3793-3872, methods @17082-17157 |
