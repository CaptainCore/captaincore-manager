...
| ~~New site › **Manual** (Connect site)~~ **CLOSED 2026-08-23** — `submitManualSite()` POSTs `/sites` with the v1 dialog_new_site contract (derived alnum slug, production + optional staging env, new protocol/port fields defaulting sftp/2222, account assignment from the dialog). Offload/env-vars stay post-connect per the form note (v3 environment editing covers them). Verified with stubbed POST: payload matches Site::create validation exactly. | `nsCreate` | was: mock, non-Kinsta onboarding impossible |
...
| ~~manual **Add Site**~~ **CLOSED 2026-08-23** — see the flow table above; offload + env-vars remain post-connect edits (deliberate, noted in the form). | full per-env credentials → `POST /sites` | was: mock |
| ~~`GET/POST/DELETE /sites/{id}/{env}/domains` + `PUT …/domains/primary`~~ **CLOSED 2026-08-23** — new **Domains** leaf under the site Inventory group (Kinsta/Rocket.net only; other providers show a "managed at the provider" note). Lists mappings with Active/Pending-DNS status and Primary/System badges, add by name, delete, and Make-primary (search-and-replace), all with the v1 payloads + refetch-after-5s ritual. `envDomainsVals`/`loadEnvDomains` in site-detail.js. Verified with stubbed provider calls: all four writes carry the correct bodies; primary + system domains protected from delete. | Kinsta/Rocket.net domain mappings | was: impossible in v3 |
| ~~`PUT /sites/{id}/settings`~~ **CLOSED 2026-08-23** — "Update settings…" action on the site overview (operator) opens a dialog: managed-updates toggle + excluded plugin/theme chips prefilled from the env's stored csv, saving with the v1 `{environment, value}` contract (server dispatches its own `site sync`). Verified with stubbed PUT: prefill honored, toggles produce the right arrays. | per-env managed-update policy | was: read-only |
...
| ~~sites-list bulk selection + `/sites/bulk-tools`~~ **CLOSED 2026-08-23** — see the headline bulk-actions row; each selected site's Production env id is POSTed to `/sites/bulk-tools`. | multi-site tools | was: mock |
