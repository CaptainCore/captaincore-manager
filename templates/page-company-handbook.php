		...
		<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			...
			<div class="entry-content">
			    ...
			    foreach( $terms as $term ) : ?>
			    	...
			    	<?php $manager = get_field('manager', 'process_role_'. $term->term_id); 
			    ...
			    if( $process_logs ): ?>
			    	...
			    	<?php foreach( $process_logs as $process_log ): 
			    		...
			    		$desc = get_field('description', $process_log->ID);
			    	?>
			    		<div class="process-star" data-post-id="<?php echo $process_log->ID; ?>" data-weekday="<?php echo $weekday; ?>" data-yearweek="<?php if ($weekday == 0) { 
			    			echo "$year-$week_sunday";
			    		} else {
			    			echo "$year-$week";
			    		 } ?>">
			    		<span class="info">
			    			<i class="fa fa-calendar" aria-hidden="true"></i> <?php echo get_the_date( "M j | g:ia", $process_log->ID ); ?>
			    			
			    			<a href="#" class="process-log-completed alignright"><i class="fa fa-check" aria-hidden="true"></i></a>
			    			<a href="<?php echo get_edit_post_link( $process_log->ID ); ?>" class="alignright"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
			    			<i class="fa fa-link" aria-hidden="true"></i> <a href="<?php echo get_permalink( $process_id ); ?>"><?php echo get_the_title( $process_id ); ?></a> 
			    			<?php if ($desc) { ?>
								<div class="desc"> 
									<?php echo WPCom_Markdown::get_instance()->transform( $desc, array('id'=>false,'unslash'=>false)); ?>
								</div>
			    			...
			    			<?php if (get_field('website', $process_log->ID)) { 
			    				$website = get_field('website', $process_log->ID);
			    				foreach( $website as $p ): // variable must NOT be called $post (IMPORTANT) ?>
			    				<i class="fa fa-link" aria-hidden="true"></i> <a href="<?php echo get_edit_post_link( $p ); ?>"><?php echo get_the_title( $p ); ?></a>
			    				<?php endforeach;  
			    			} ?>
