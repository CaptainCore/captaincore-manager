...
class SiteCPT {
    ...
    public function get() {
        ...
        if ( $site_details->provider == 'kinsta' and $environments[0]->database_username ) {
            $address_array = explode( ".", $environments[0]->address );
        ...
        if ( $site_details->provider == 'kinsta' and $environments[1]->database_username ) {
            $address_array = explode( ".", $environments[1]->address );
            $kinsta_ending = array_pop( $address_array );
            if ( $kinsta_ending != "com" && $kinsta_ending != "cloud" ) {
                $kinsta_ending = "cloud";
            }
    ...
    public function get_raw( $new_id = "" ) {
        ...
        $environments    = $db_environments->fetch_environments( $site->ID );
        if ( $new_id != "" ) {
            $environments    = $db_environments->fetch_environments( $new_id );
        }
        ...
        $core                = $environments[0]->core;
        $mailgun             = get_field( 'mailgun', $site->ID );
        $key                 = get_field( 'key', $site->ID );

        // Prepare site details to be returned
        $site_details                       = new \stdClass();
        $site_details->created_at           = get_the_date( 'Y-m-d H:i:s', $site->ID );
        $site_details->name                 = $domain;
        $site_details->site                 = get_field( 'site', $site->ID );
        $site_details->provider             = get_field( 'provider', $site->ID );
        $site_details->token                = get_field( 'token', $site->ID );
        $site_details->status               = get_field( 'status', $site->ID );
        $site_details->details           = json_encode( [ 
            "subsites"    => isset( $subsite_count ) ? $subsite_count : '',
            "storage_raw" => isset( $storage ) ? $storage : '',
            "storage"     => isset( $storage_gbs ) ? $storage_gbs : '',
            "visits"      => isset( $visits ) ? $visits : '',
            "mailgun"     => isset( $mailgun ) ? $mailgun : '',
            "core"        => isset( $core ) ? $core : '',
            "key"         => isset( $key ) ? $key : '',
        ] );

        if ( $customer ) {
            foreach ( $customer as $customer_id ) {
                $site_details->account_id = $customer_id;
            }
        }

        if ( $site_details->status != "active" ) {
            $site_details->status = "inactive";
        }
