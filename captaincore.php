...
// Register Custom Post Type
function worker_post_type() {

	$labels = array(
		'name'                  => _x( 'Workers', 'Post Type General Name', 'captaincore' ),
		'singular_name'         => _x( 'Worker', 'Post Type Singular Name', 'captaincore' ),
		'menu_name'             => __( 'Worker', 'captaincore' ),
		'name_admin_bar'        => __( 'Worker', 'captaincore' ),
		...
		'edit_item'             => __( 'Edit Worker', 'captaincore' ),
		'update_item'           => __( 'Update Worker', 'captaincore' ),
		'view_item'             => __( 'View Worker', 'captaincore' ),
		'view_items'            => __( 'View Workers', 'captaincore' ),
		'search_items'          => __( 'Search Worker', 'captaincore' ),
		'not_found'             => __( 'Not found', 'captaincore' ),
		'not_found_in_trash'    => __( 'Not found in Trash', 'captaincore' ),
		'featured_image'        => __( 'Featured Image', 'captaincore' ),
		'set_featured_image'    => __( 'Set featured image', 'captaincore' ),
	...
	$args = array(
		'label'                 => __( 'Worker', 'captaincore' ),
		'description'           => __( 'Worker Description', 'captaincore' ),
		'labels'                => $labels,
		'supports'              => array( 'title', 'editor' ),
		'hierarchical'          => false,
		'public'                => true,
	...
	register_post_type( 'anchorworker', $args );
...
function anchor_install_action_callback() {
	...
	if ($cmd == "remove") {
		$command = "captaincore config delete $install $domain ".
		($homedir ? "--homedir=$homedir " : "" );
	}
	if ($cmd == "backup") {
		date_default_timezone_set('America/New_York');
	...
	if ($cmd == "snapshot") {
		...
		$timestamp = date("Y-m-d-hms",$t);
		if($value) {
			$command = "captaincore snapshot $domain --email=$value > ~/Tmp/$timestamp-snapshot_$install.txt 2>&1 & sleep 5; head ~/Tmp/$timestamp-snapshot_$install.txt";
		} else {
			$command = "captaincore snapshot $domain > ~/Tmp/$timestamp-snapshot_$install.txt 2>&1 & sleep 5; head ~/Tmp/$timestamp-snapshot_$install.txt";
		}
...
class PageTemplater {
	...
	private function __construct() {
		...
		$this->templates = array(
			'templates/page-anchor-api.php' => 'Anchor API',
